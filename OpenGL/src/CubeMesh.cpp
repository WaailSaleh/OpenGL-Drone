...
CubeMesh newCube()
{
	...
	cube.floors = 1;
	cube.position[2] =(GLfloat) 0;
	cube.position[1] = (GLfloat) 1;
	cube.position[0] = (GLfloat) 0;
    return cube;
}
...
void cubeTransform(CubeMesh *cube, GLfloat scalex, GLfloat scalez, GLfloat height) {
	cube->sfx = scalex;
	cube->sfz = scalez;
	cube->height = height;
...
void drawCube(CubeMesh *cube)
{
	...
	{	
		...
		// ......
		//glTranslatef(cube->position[0], 0, cube->position[2]);
	...
	for (int j = 0; j < 8; j++)
		vertices[j][1] -= cube->floors * 2;

}
void moveCube(CubeMesh *cube, GLfloat x, GLfloat z) {
	cube->position[0] += x;
	cube->position[2] += z;
